...
df_per = read_pkg_out(
    os.path.join("out_per", "d_out_patient_across_class_10s_seglength_480_all.pkl")
)
...
for sub in subs:
    ...
    for idx, loc_ in enumerate(["_cortex", "subcortex"]):
        cols_ = [
            c for c in df_sub.columns if "welch_psd" in c and "mean" in c and loc_ in c
        ]  #
        ...
        bk = df_sub.pkg_bk.values / df_sub.pkg_bk.values.max()
        tremor = df_sub.pkg_tremor.values / df_sub.pkg_tremor.values.max()
        ...
        plt.plot(
            bk * 10 + len(cols_) - 0.5, color="red", label="PKG Bradykinesia score"
        )
        plt.plot(tremor * 10 + len(cols_) - 0.5, color="blue", label="PKG Tremor score")
        ...
        per_ = np.round(
            df_per.query("sub == @sub and loc == @str_search_loc")["ba"].iloc[0], 2
        )
        plt.legend()
