...
if __name__ == "__main__":
    ...
    df_["model"] = "LM"
    df = pd.concat([df, df_])
    df_ = read_pkg_out(os.path.join("out_per", "d_out_patient_across_class_10s_seglength_480_all_XGB.pkl"))
    df_["model"] = "XGB"
    df = pd.concat([df, df_])
    df_ = read_pkg_out(os.path.join("out_per", "d_out_patient_across_class_10s_seglength_480_all_PCALM.pkl"))
    df_["model"] = "PCA_LM"
    df = pd.concat([df, df_])
    df_ = read_pkg_out(os.path.join("out_per", "d_out_patient_across_class_10s_seglength_480_all_CEBRA.pkl"))
    df_["model"] = "CEBRA"
    df = pd.concat([df, df_])
    df_ = read_pkg_out(os.path.join("out_per", "d_out_patient_across_class_10s_seglength_480_all_RF.pkl"))
    df_["model"] = "RF"
    df = pd.concat([df, df_])
    ...
    plt.savefig("figures_ucsf/ML_model_comparison.pdf")
    plt.show(block=True)
