...
if __name__ == "__main__":
    ...
    for label_name in ["pkg_dk", "pkg_bk", "pkg_tremor"]:
        print(label_name)
        d_out[label_name] = {}
        pdf_pages = PdfPages(os.path.join("figures_ucsf", f"decoding_across_patients_{label_name}.pdf")) 
        ...
        for sub_test in tqdm(subs):
            ...
            df_test = df_all[df_all["sub"] == sub_test]

            df_test = df_test.drop(columns=["sub"])
            y_test = np.array(df_test[label_name])
            df_train = df_all[df_all["sub"] != sub_test]
            df_train = df_train.drop(columns=["sub"])
            y_train = np.array(df_train["pkg_dk"])
            ...
            d_out[label_name][sub_test] = {}
            d_out[label_name][sub_test]["corr_coeff"] = corr_coeff
            d_out[label_name][sub_test]["r2"] = metrics.r2_score(y_test, pr)
            d_out[label_name][sub_test]["mse"] = metrics.mean_squared_error(y_test, pr)
            d_out[label_name][sub_test]["mae"] = metrics.mean_absolute_error(y_test, pr)
            d_out[label_name][sub_test]["pr"] = pr
            d_out[label_name][sub_test]["y_"] = y_test
            d_out[label_name][sub_test]["time"] = df_test["pkg_dt"].values
            d_out[label_name][sub_test]["feature_importances"] = feature_importances
