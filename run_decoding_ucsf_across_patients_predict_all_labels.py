...
if __name__ == "__main__":
    ...
    for loc_ in ["ecog_stn", "ecog", "stn",]:
        ...
        if PLT_:
            pdf_pages = PdfPages(os.path.join("figures_ucsf", f"decoding_across_patients_class_{label_name}_{loc_}_10s_segmentlength_all_{MODEL_NAME}_all_labels.pdf")) 
        if loc_ == "ecog_stn":
            df_use = df_all.copy()
        elif loc_ == "ecog":
        ...
        for sub_test in subs:  # tqdm(
            ...
            y_test = np.array(df_test[["pkg_dk_class", "pkg_bk_class", "pkg_tremor_class"]])
            df_train = df_use[df_use["sub"] != sub_test]
            df_train = df_train.drop(columns=["sub"])
            #y_train = np.array(df_train[label_name])
            y_train = np.array(df_train[["pkg_dk_class", "pkg_bk_class", "pkg_tremor_class"]])
            ...
            if CLASSIFICATION:
                ...
                if MODEL_NAME == "CB":
                    model = CatBoostClassifier(silent=False, loss_function='MultiLogloss')  # class_weights=class_weights
                elif MODEL_NAME == "LM":
