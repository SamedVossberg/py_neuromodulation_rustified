...
MODEL_NAME = "CEBRA"  # "CB", "LM", "XGB", "PCA_LM", "CEBRA", "RF"

if __name__ == "__main__":
    PATH_DATA = r"C:\Users\ICN_GPU\Downloads\all_merged_normed.csv"
    ...
    for loc_ in [
        ...
        if PLT_:
            pdf_pages = PdfPages(
                os.path.join(
                    "figures_ucsf",
                    f"decoding_across_patients_class_{label_name}_{loc_}_10s_segmentlength_all_{MODEL_NAME}_all_labels.pdf",
                )
            )
        if loc_ == "ecog_stn":
            df_use = df_all.copy()
        ...
        for sub_test in subs:  # tqdm(
            ...
            if CLASSIFICATION:
                ...
                model = CatBoostRegressor(silent=True, task_type="GPU", devices="0")  #
            ...
            if CLASSIFICATION:
                ...
                pr = pr.astype(int)
                d_out[label_name][loc_][sub_test]["accuracy"] = metrics.accuracy_score(
                    y_test, pr
                )
                d_out[label_name][loc_][sub_test]["pr_proba"] = model.predict_proba(
                    X_test
                )
                ...
                d_out[label_name][loc_][sub_test]["mse"] = metrics.mean_squared_error(
                    y_test, pr
                )
                d_out[label_name][loc_][sub_test]["mae"] = metrics.mean_absolute_error(
                    y_test, pr
                )
            ...
            if PLT_:
                ...
                plt.plot(
                    d_out[label_name][loc_][sub_test]["pr_proba"][:, 1],
                    label="pr_proba",
                )
                plt.plot(
                    d_out[label_name][loc_][sub_test]["true_reg_normed"].values,
                    label="true",
                )
                ...
                if CLASSIFICATION:
                    plt.title(
                        f"ba: {np.round(d_out[label_name][loc_][sub_test]['ba'], 2)} sub: {sub_test}"
                    )
                else:
    ...
    if CLASSIFICATION:
        SAVE_NAME = f"d_out_patient_across_class_10s_seglength_480_all_{MODEL_NAME}_all_labels_wo_spec_CEBRA.pkl"
    else:
