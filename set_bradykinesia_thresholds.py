...
if __name__ == "__main__":
    ...
    pdf_pages = PdfPages(os.path.join("figures_ucsf", "brad_labels.pdf")) 
    ...
    for sub in np.sort(subs):
        ...
        plt.subplot(142)
        plt.plot(df_sub["pkg_bk"].values / df_sub["pkg_bk"].values.max())
        plt.title("Normalized PKG Bradykinesia")
        plt.subplot(141)
        plt.plot(df_sub["pkg_bk"].values)
        plt.title("Raw PKG Bradykinesia")
        plt.subplot(143)
        plt.hist(df_sub["pkg_bk"].values / df_sub["pkg_bk"].values.max(), bins=50)
        plt.title("Histogram PKG Bradykinesia")
        ...
        plt.title("Histogram PKG Bk \nClipped 0.02")
        plt.suptitle(f"{sub} set threshold: {thresholds_[sub]}")
        plt.tight_layout()

        pdf_pages.savefig(plt.gcf())
        plt.close()
        df_sub["pkg_bk_class"] = df_sub["pkg_bk_normed"] > thresholds_[sub]
        l_.append(df_sub)
    pdf_pages.close()
